...
public class Word_Result extends AppCompatActivity {
    ...
    String word,sword1,sword2,sword3,sword4;
    int save_visibility;
    String example;
...
// finally change the color
        ...
        if (extras != null) {
            ...
            if((catimportance>50 || greimportance>50) && word.length()>3  ){
                ...
                if(random==1)
                    mDatabase.child("word1").setValue(word.toLowerCase());
                else if(random==2)
                    mDatabase.child("word2").setValue(word.toLowerCase());
                else if(random==3)
                    mDatabase.child("word3").setValue(word.toLowerCase());
                else
